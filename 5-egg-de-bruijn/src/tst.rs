...
impl Realization for Expr {
    ...
    fn simplify(&self, steps: u32) -> Self {
        ...
        let runner = Runner::default().with_scheduler(SimpleScheduler).with_iter_limit(steps as usize).with_expr(&self.0).run(&rewrites);
    ...
    fn find_eq(&self, other: &Self, steps: u32) -> bool {
        ...
        let rewrites = [beta_reduction()];
        let runner = Runner::default().with_scheduler(SimpleScheduler).with_iter_limit(steps as usize).with_egraph(eg).run(&rewrites);
