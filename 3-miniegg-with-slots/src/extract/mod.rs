...
pub fn extract<L: Language, CF: CostFn<L>>(i: Id, eg: &EGraph<L>) -> RecExpr<L> {
    ...
    let mut map: HashMap<Id, WithOrd<RecExpr<L>>> = HashMap::default();
    let mut queue: BinaryHeap<WithOrd<L>> = BinaryHeap::new();

    for id in eg.ids() {
        for x in eg.enodes(id) {
            if x.applied_id_occurences().is_empty() {
                let c = CF::cost(&x, |_| panic!());
                queue.push(WithOrd(x, c));
            }
        }
    }

    while let Some(WithOrd(enode, c)) = queue.pop() {
        let re = extract_step(enode.clone(), &map);
        let i = eg.lookup(&enode).unwrap();
        map.insert(i.id, WithOrd(re, c));
...
fn extract_step<L: Language>(enode: L, map: &HashMap<Id, WithOrd<RecExpr<L>>>) -> RecExpr<L> {
    let mut c = enode.clone();
    let mut re = RecExpr::empty();
    for x in c.applied_id_occurences_mut() {
        let re2 = map[&x.id].0.clone();
        re.extend(re2);
        x.id = re.head_id();
    }
    re.push(c);
